...
EMERGENCY_BACKUP_MS = 260        # Backup duration after an emergency stop (milliseconds)

# --- PATH FOLLOWING ---
NAV_WAYPOINT_STRIDE = 8          # Sample every Nth path point as a waypoint
NAV_WAYPOINT_REACH_PX = 6        # Distance (pixels) to consider a waypoint reached
NAV_GOAL_REACH_PX = 12            # Distance (pixels) to consider the final goal reached
NAV_MAX_REPLANS = 20             # Max plan/replan attempts per navigation goal
NAV_MAX_STEPS_PER_GOAL = 600000  # Hard cap on control steps per navigation goal
NAV_MAP_UPDATE_EVERY = 2         # Update the occupancy grid every Nth control step
NAV_STUCK_DIST_M = 0.05          # Movement (meters) below which progress is "stuck"
NAV_STUCK_PATIENCE = 30          # Control ticks without progress before declaring stuck
...
COLUMN_REACH_PX = 7              # Map-distance (pixels) at which a KNOWN pillar counts as reached
COLUMN_REFINE_MIN_PIXELS = 300   # Pixel count above which a pillar estimate is refined
...
DEPTH_OBST_MIN_M = 0.1          # Ignore returns closer than this (self / noise)
DEPTH_OBST_MAX_M = 1           # Ignore returns farther than this (far data is unreliable)
DEPTH_COLLISION_Z_MIN = 0.01     # Min height (m) above floor to count as an obstacle
DEPTH_COLLISION_Z_MAX = 0.25     # Max height (m); above this the robot passes under
DEPTH_FLOATING_Z_MIN = 0.11      # If a cell's LOWEST return is above this -> floating wall
DEPTH_OBST_MIN_POINTS = 10        # Min projected points per cell to accept it (noise gate)
