...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def update_confidence(self, robot_pos, lidar_map_points):
        """Update per-cell confidence (CONFIDENCE_MIN-CONFIDENCE_MAX)
        Hits increase confidence (+CONFIDENCE_HIT_INC), passes-through decrease
        it (-CONFIDENCE_MISS_DEC). 
        """
    ...
    def emergency_clear(self, value = 10):
        for x in range(MAP_SIZE):
            for y in range(MAP_SIZE):
                ...
                if self.grid_map[y,x] == OBSTACLE or self.grid_map[y,x] == CLOSED or self.grid_map[y,x] == GREEN_CARPET or self.grid_map[y,x] == FLOATING_WALL:
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def update_grid_map(self):
        ...
        - confidence > CONFIDENCE_WALL_THRESHOLD -> OBSTACLE
        - confidence == CONFIDENCE_INITIAL -> UNKNOWN
        - otherwise -> FREESPACE
        """
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def mark_blocked_ahead(self, forward_m=0.15, radius_m=0.1, value=CLOSED):
        ...
        blocked disc is treated as obstacle so that the robot remaps avoiding it 
        ...
        for yy in range(y0, y1 + 1):
            for xx in range(x0, x1 + 1):
                ...
                if self.grid_map[yy, xx] == FLOATING_WALL:
                    continue
