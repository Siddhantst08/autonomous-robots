...
class MyRobot(Robot):
    ...
    def __init__(self):
        ...
        self.accOdo = AccOdo()
        self.motorVelocity = MotorVelocity(self.wheel_sensors)
# REFERENCE: 
# Source: Gemini 3.1 Pro with detailed prompting and adapted to the teams use case.
    ...
    def _init_camera_model(self):
        ...
        # Depth-camera intrinsics (may differ from the RGB camera). Used by the depth obstacle layer that catches flat-on-floor and floating walls.
        try:
        ...
        # Ground-projection extrinsics for green detection
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def robot_on_ground(self, max_tan_pitch=0.20):
        ...
        large tilt like climbing a wall is rejected so bad scans are skipped.
        """
        try:
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def turn_by(self, degrees, direction='left', speed=2.0):
        """Rotate in place by an angle using drift-free compass feedback for initail mapping rotation and stuff like that
        """
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def turn_to_heading(self, target_rad, tol=0.12, max_iters=250, speed=6.0):
        """Rotate in place to face an absolute world heading.
        """
...
# Source: Gemini 3.1 Pro with detailed prompting and adapted to the teams use case.
    ...
    def follow_local_target(self, map_target):
        ...
        compVal = self.imu["compass"].getValues() 
        if abs(compVal[2]) > 0.1:
            print("Tilted!")
            is_stuck = True
            return False, is_stuck

        # leftV = (self.motors['fl'].getVelocity() + self.motors['rl'].getVelocity()) * 0.5
        # rightV = (self.motors['fr'].getVelocity() + self.motors['rr'].getVelocity()) * 0.5
        actualV = self.motorVelocity.getVelocity()
        leftV = (actualV[0] + actualV[2]) * 0.5
        rightV = (actualV[1] + actualV[3]) * 0.5
        if leftV == 0 and rightV == 0:
            self.accOdo.velocity = np.array([0.0, 0.0, 0.0])
        avgV = (leftV + rightV) * 0.5

        accVal = self.imu["accelerometer"].getValues()
        init = self.imu["inertial_unit"].getRollPitchYaw()
        self.accOdo.update(accVal,init)
        if abs(avgV) <= 0.1:
            self.accOdo.velocity = np.array([0.0, 0.0, 0.0])
        th = 0.01
        # print(f"{avgV = }")
        # print(f"{abs(self.accOdo.velocity[0]) = :.3f} , {abs(avgV)*0.1 - th = :.3f}")
        if abs(self.accOdo.velocity[0]) < abs(avgV)*0.1 - th and (avgV) >= 0.1:
        # print(f"{accVal[0] = :.3f}, {avgV = :.3f}")
            # print(f"{avgV = }")
            # print(f"{abs(self.accOdo.velocity[0]) = } , {abs(avgV)*0.1 - th = }")
            print("Collision1!")
            is_stuck = True
            return False, is_stuck
        # if abs(accVal[0]) > 3 and abs(avgV) > 6:
        #     print("Collision2!")
        #     is_stuck = True
        #     return False, is_stuck
...
class AccOdo:
    ...
    def update(self, accVal:list, rollPitchYaw:list):
        ...
        accel_x, accel_y, accel_z = accVal
        accel_raw = np.array([accel_x, accel_y, accel_z])

        # Rotation matrix R: world → body (ZYX order)
        roll, pitch, yaw = rollPitchYaw
        cr, sr = np.cos(roll), np.sin(roll)
        cp, sp = np.cos(pitch), np.sin(pitch)
        cy, sy = np.cos(yaw), np.sin(yaw)
        
        # R = Rz(yaw) * Ry(pitch) * Rx(roll)   → actually the transpose for body-from-world
        # Full rotation matrix from body to world:
        ...
        # Transform to body frame: g_body = R_body_to_world.T @ g_world
        grav = R_body_to_world.T @ g_world
        accel = accel_raw - grav
        ...
        if print:
            toPrint = accel_raw
            print(f"{toPrint[0]:.4f}, {toPrint[1]:.4f}, {toPrint[2]:.4f}")
            toPrint = grav
            print(f"{toPrint[0]:.4f}, {toPrint[1]:.4f}, {toPrint[2]:.4f}")
            toPrint = accel
            print(f"{toPrint[0]:.4f}, {toPrint[1]:.4f}, {toPrint[2]:.4f}")
...
class MotorVelocity():
    def __init__(self,motorSensors:dict[str,PositionSensor]):
        self.motorSensors = motorSensors
        self.lastValues = [s.getValue() for s in motorSensors.values()]
    ...
    def update(self):
        ...
        dt = current_time - self.last_time
        self.last_time = current_time
        self.currentValues = [s.getValue() for s in self.motorSensors.values()]
        self.velocity = [(cur - prev)/dt for (cur,prev) in zip(self.currentValues,self.lastValues)]
        self.lastValues = self.currentValues
        # print(self.lastValues)

    def getVelocity(self):
        self.update()
        return self.velocity
## REFERENCE: Classes AccOdo and MotorVelocity are original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
