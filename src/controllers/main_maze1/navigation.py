...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def _stamp_depth(self):
        """Stamp depth-camera obstacles the horizontal lidar cannot see, so planning and DWA avoid them."""
        try:
...
# Source: Gemini 3.1 Pro with detailed prompting and adapted to the teams use case.
    ...
    def _optional_reverse(self):
        """Tiny backward nudge if RECOVERY_ALLOW_REVERSE is enabled.
        """
...
# REFERENCE: Original code authored by the project team. No external sources or LLMs were used. Values are calibrated for best performance.
    ...
    def _nudge_rotate(self, rotate = False):
        """Rotate in place toward the more open side to change the viewpoint and break a deadlock WITHOUT driving in reverse; the caller then replans.
        """
        robot = self.robot
        robot.stop_motor()
        self._optional_reverse()                     
