...
class Navigator:
    ...
    def __init__(self, robot:MyRobot, perception:Perception):
    ...
    def navigate_to(self, goal_map, planner, reach_px=NAV_GOAL_REACH_PX,
                    ...
                    max_steps=NAV_MAX_STEPS_PER_GOAL, is_pillar = False):
    ...
    def _follow_waypoints(self, waypoints, goal_map, reach_px, tick_cb,
        ...
        for index,wp in enumerate(waypoints):
            ...
            while steps < step_budget:    
                # for w in waypoints[index:min(len(waypoints),index + 5)]:
                # print(wp)
                # mapstatus = self.robot.map_object.grid_map[wp[0],wp[1]]
                # mapconf = self.robot.map_object.confidence[wp[0],wp[1]]
                # print(f"next waypoint is: {mapstatus}, confidence: {mapconf}")
                # if mapstatus not in [FREESPACE, UNKNOWN] and mapconf > CONFIDENCE_WALL_THRESHOLD:
                #     print("aborting next waypoint")
                #     self.robot.map_object._draw_point(wp[0],wp[1],(255,0,0))
                #     return 'replan', steps
                cur_time = time.time()
                replan_sec = 4
                if cur_time - start_time > replan_sec:
                    print(f"replan after {replan_sec}s")
                    return 'replan', steps
                ...
                if self._emergency_check(allow_green_avoid):
                    ...
                    robot.map_object.mark_blocked_ahead(forward_m= 0.1,radius_m=0.1)
                    # print("replan1")
                    return 'replan', steps

                if robot.get_map_distance(wp) < NAV_WAYPOINT_REACH_PX:
                    # print("break, waypoint reached")
                ...
                if is_stuck:
                    robot.stop_motor()
                    self._nudge_rotate(back_duration= EMERGENCY_BACKUP_MS + 200)
    ...
    def _optional_reverse(self, duration = EMERGENCY_BACKUP_MS):
        ...
        if RECOVERY_ALLOW_REVERSE:
            self.robot.move_backward_milisecond(duration)

    def _nudge_rotate(self, rotate = False, back_duration = None):
        ...
        robot.stop_motor()
        if back_duration:
            self._optional_reverse(back_duration) 
        else:  
