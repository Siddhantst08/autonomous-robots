...
# --- MOTION CONTROL ---
MOTOR_VELOCITY_FORWARD = 4                 # [USED] Forward velocity during random exploration
...
COLUMN_MAX_LOCALIZE_M = 3.5      # Max depth (meters) at which a pillar is localized
COLUMN_REACH_DIST_M = 0.55       # (unused) legacy lidar reach distance; reach now uses map coords
COLUMN_REACH_PX = 8 #6             # Map-distance (pixels) at which a KNOWN pillar counts as reached
COLUMN_REFINE_MIN_PIXELS = 2400   # Pixel count above which a pillar estimate is refined

# --- GREEN (POISON) HAZARD ---
GREEN_AHEAD_MIN_PIXELS = 500     # Green pixels in the lower-central view to react to
GREEN_STOP_DEPTH_M = 0.6         # Depth (meters) below which green ahead triggers avoidance
GREEN_MARK_MIN_POINTS = 10        # Minimum projected points to stamp green onto the grid
...
DEPTH_OBST_MAX_M = 2         # Ignore returns farther than this (far data is unreliable)
